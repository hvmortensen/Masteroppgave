...
for i in range(N):                                  # gå igennem alle målepunkter på perspexpladen: range(N) = 0,1,2,...29
    ...
    for j in range(n):                              # gå igennem alle målinger i hvert punkt: range(n) = 0,1,2
        AllPoints[j] = m[i + N*j]                   # alle målinger fra alle forsøg
    ...
    SE[i] = np.std(AllPoints)/np.sqrt(n*m.shape[1]) # udregn SE og lav N-array med SEM i hvert målepunkt
...
for i in range(y_lines):                            # gå igennem hver linje på perspexpladen
    for j in range(x_points):                       # gå igennem hvert punkt på perspexpladen
        y_all[j]  = Mean_Of_Total[j + i*(x_points)] # fyld 6 arrays med gennemsnittene af alle målinger
        y_means[j] = Mean_Of_Means[j + i*(x_points)]# fyld 6 arrays med gennemsnit af gennemsnittene fra hver forsøg
        y_se[j]  = SE[j + i*(x_points)]             # fyld 6 arrays med standard error (SE)
        y_sem[j] = SEM[j + i*(x_points)]            # fyld 6 arrays med standard error of the mean (SEM)
    DoseMatrix_all[i] = y_all                       # fyld 6x5-dosematrix til plotting
    DoseMatrix_mean[i] = y_means                    # fyld 6x5-dosematrix til plotting
    SE_Matrix[i] = y_se                             # fyld 6x5-SE-matrix til plotting
    SEM_Matrix[i] = y_sem                           # fyld 6x5-SEM-matrix til plotting
...
#### I DEN FØRSTE MÅLING
norm = np.zeros(m.shape)                            # norm
norm_wi = np.zeros(m.shape)                         # norm without index i
NormArray = np.zeros( (m.shape[1], m.shape[0]) )    # NormArray
NormArray_wi = np.zeros( (m.shape[1], m.shape[0]) ) # NormArray without index i
...
first_meas_norms = np.array([1.0242, 1.0229, 1.0153, 1.0110 ])
...
if n == 1:
    figs, axs = plt.subplots(1,2,figsize=(7,4.5))
    figs.suptitle("Målt dosis med standardafvig, %s målinger pr punkt, %s, %ss"%(nx1, SSD, t), fontsize=FS)
    ...
    for i in range(x_points):
        for j in range(y_lines):
            c = A_matrix[0,j,i]
            d = O_matrix[0,j,i]
            axs[0].text(i, j, '%.2f' %c, va='bottom', ha='center', fontsize=fs)
            axs[0].text(i, j, "±"'%.2f' %d, va='top', ha='center', fontsize=fs)
            axs[1].text(i, j, '%.2f' %d, va='center', ha='center', fontsize=fs)
else:
    figs, axs = plt.subplots(nrows=2,ncols=n,figsize=(12, 8))
    figs.suptitle("Målt dosis med standardafvig fra hvert forsøg, %s målinger pr punkt, %s, %ss"%(nx1, SSD, t), fontsize=FS)
    for i in range(n):
        ...
        for j in range(x_points):
            for k in range(y_lines):
                c = A_matrix[i,k,j]
                d = O_matrix[i,k,j]
                axs[0,i].text(j, k, '%.2f' %c, va='bottom', ha='center', fontsize=fs)
                axs[0,i].text(j, k, "±"'%.2f' %d, va='top', ha='center', fontsize=fs)
                axs[1,i].text(j, k, '%.2f' %d, va='center', ha='center', fontsize=fs)
...
fig.suptitle("Gennemsnitsdosis og standard error, %s $\\times$ %s målinger pr punkt, %s, %ss"%(n, nx1, SSD, t), fontsize=FS)
...
for i in range(x_points):
    for j in range(y_lines):
        ...
        g = np.array([c, d, e])
        for k in range(3):
            ax[k].text(i, j, '%.2f' %g[k], va='center', ha='center', fontsize=fs)
...
#     figo.suptitle(tito%(SSD, t), fontsize=FS)
...
#         axo[i].legend(loc=9, fontsize=fs)
...
#             axo[i,j].axhline(y=k,label="gennemsnit: %.4f"%k, color="purple")
#             axo[i,j].plot(NormArray[j + i*5], ".", label="målinger")
#             axo[i,j].set_title("alle %s.-målinger"%(j + 1 + i*5))
#             axo[i,j].legend(loc=9, fontsize=fs)
...
# plt.savefig("%s_%s_norm_alle.pdf"%(SSD,t))
#
#
#
#
...
# axi.axhline(y=k,label="gennemsnitlig førstemåling ift. gennemsnittet: %.4f"%k, color="purple")
...
# plt.savefig("%s_%s_norm.pdf"%(SSD,t))
#


#### PLOT FOR AT SE HVOR MEGET ENKELTE MÅLINGER AFVIGER FRA GENNEMSNITTET AF DE RESTERENDE MÅLINGER

titq = "Målingsindeks normaliseret i forhold til gennemsnittet af de resterende målinger i hvert punktmålingssæt, %s, %ss"
ylimits = (0.91, 1.09)
if m.shape[1] <= 5:
    figq, axq = plt.subplots(nrows=1,ncols=5,figsize=(14, 6), sharey="all")
    figq.suptitle(titq%(SSD, t), fontsize=FS)
    for i in range(m.shape[1]):
        k_wi = np.mean(NormArray_wi[i])
        axq[i].axhline(y=k_wi,label="gennemsnit: %.4f"%k_wi, color="red")
        axq[i].plot(NormArray_wi[i], ".", label="målinger")
        axq[i].set_title("alle %s.-målinger"%(i+1))
        axq[i].legend(loc=9, fontsize=f)
        axq[i].set_ylim((ylimits))
        axq[i].set_xticks(np.linspace(0,int(n*N),10))
else:
    figq, axq = plt.subplots(nrows=2,ncols=5,figsize=(14, 8), sharey="all")
    figq.suptitle(titq%(SSD, t), fontsize=FS)
    for i in range(2):
        for j in range(5):
            k_wi = np.mean(NormArray_wi[j + i*5])
            axq[i,j].axhline(y=k_wi,label="gennemsnit: %.4f"%k_wi, color="red")
            axq[i,j].plot(NormArray_wi[j + i*5], ".", label="målinger")
            axq[i,j].set_title("alle %s.-målinger"%(j + 1 + i*5))
            axq[i,j].legend(loc=9, fontsize=fs)
            axq[i,j].set_ylim(ylimits)
            axq[i,j].set_xlabel("Målingsrække")
            axq[i,j].set_ylabel("Normaliseret måling")
            axq[i,j].set_xticks(np.linspace(0,int(n*N),7))
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
...
titr = "Kalibreringsfaktor for kort eksponering"
figr, axr = plt.subplots()
axr.plot(scope,first_meas_norms, "r", label="$\\delta(t)$")
axr.set_xlabel("Eksponeringstid (s)")
axr.set_ylabel("Kalibreringsfaktor")
axr.set_title(titr)
axr.legend()
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
